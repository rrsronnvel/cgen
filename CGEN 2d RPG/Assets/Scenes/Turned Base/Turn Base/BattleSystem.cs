...
public class BattleSystem : MonoBehaviour
{
    ...
    IEnumerator PlayerSpecialAttack()
    {
        ...
        if (specialAttackCount <= 0)
        {
            ...
            yield return new WaitForSeconds(2f); // Add this line to add a delay

            // Switch to the enemy's turn
            state = BattleState.ENEMYTURN;
            StartCoroutine(EnemyTurn()); // Add this line to start the enemy's turn
        }
        else
        {
            // Get the Animator component from the player
